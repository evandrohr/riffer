...
describe Riffer::Voice::Adapters::DeepgramVoiceAgent do
  it "bridges runtime scheduling through task_resolver and delegates operations" do
    ...
    driver = nil
    adapter = Riffer::Voice::Adapters::DeepgramVoiceAgent.new(
    ...
    expect(driver.model).must_equal TestSupport::VoiceModels::DEEPGRAM_MODEL
  ...
  it "selects thread websocket transport for background runtime" do
    ...
    driver = nil
    Riffer::Voice::Adapters::DeepgramVoiceAgent.new(
      model: TestSupport::VoiceModels::DEEPGRAM_MODEL,
      runtime_executor: runtime,
      driver_factory: lambda { |**kwargs|
        driver = TestSupport::Voice::DriverDouble.new(**kwargs)
    ...
    with_overridden_transport_connects(thread_connect: thread_connect, async_connect: async_connect) do
      selected = driver.transport_factory.call(url: "wss://example.test/deepgram", headers: {"Authorization" => "token key"})
    end
