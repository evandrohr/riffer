...
# Emitted when the provider reports input (user) transcription.
class Riffer::Voice::Events::InputTranscript < Riffer::Voice::Events::Base
  # Transcribed text.
  ...
  def initialize(text:, is_final: nil, metadata: {}, role: :user)
    super(role: role)
    @text = text
    @is_final = is_final
    @metadata = metadata
